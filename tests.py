from MiniRedis import Redis
import time

def main():
    miniRedis = Redis.Redis(4)
    
    print("---set---")
    status = miniRedis.set("teste1", 1)
    print(status)
    status = miniRedis.set("teste2", 2)
    print(status)
    status = miniRedis.set("teste3", 3)
    print(status)
    status = miniRedis.set("teste4", 4)
    print(status)
    status = miniRedis.set("teste4", 44)
    print(status)

    print("---set-ex---")
    status = miniRedis.set("teste5", 5, "EX", 2)
    print(status)
    status = miniRedis.set("teste1", 11, "EX", 4)
    print(status)
    status = miniRedis.set("teste2", 22, "EX", 8)
    print(status)

    time.sleep(10)

    print("---get---")
    value = miniRedis.get(3)
    print(value)
    value = miniRedis.get(10)
    print(value)
    value = miniRedis.get("teste1")
    print(value)
    value = miniRedis.get("teste2")
    print(value)
    value = miniRedis.get("teste4")
    print(value)
    value = miniRedis.get("teste5")
    print(value)

    print("---delete---")
    removed = miniRedis.delete("teste14")
    print(removed)
    value = miniRedis.get("teste14")
    print(value)
    removed = miniRedis.delete("teste6")
    print(removed)

    print("---dbsize---")
    size = miniRedis.dbsize()
    print(size)

    print("---incr---")
    newValue = miniRedis.incr("teste10")
    print(newValue)
    newValue = miniRedis.incr("teste10")
    print(newValue)
    newValue = miniRedis.incr("teste10")
    print(newValue)
    newValue = miniRedis.incr("teste10")
    print(newValue)
    newValue = miniRedis.incr("teste10")
    print(newValue)
    removed = miniRedis.delete("teste10")
    print(removed)
    newValue = miniRedis.incr("teste10")
    print(newValue)

    print("---zadd---")
    zinsert = miniRedis.zadd("zset1", "-inf", "mil")
    print(zinsert)
    zinsert = miniRedis.zadd("zset1", 900, "mil")
    print(zinsert)
    zinsert = miniRedis.zadd("teste10", "900", "mil")
    print(zinsert)
    zinsert = miniRedis.zadd("zset2", "700", "amil")
    print(zinsert)
    zinsert = miniRedis.zadd("zset2", "1000", "amil")
    print(zinsert)
    zinsert = miniRedis.zadd("zset2", "2000", "bdoismil")
    print(zinsert)
    zinsert = miniRedis.zadd("zset2", "10000", "ctresmil")
    print(zinsert)
    zinsert = miniRedis.zadd("zset2", "4000", "dquatromil")
    print(zinsert)
    zinsert = miniRedis.zadd("zset2", "5000", "ecincomil")
    print(zinsert)
    zinsert = miniRedis.zadd("zset2", "4500", "fseismil")
    print(zinsert)

    zinsert = miniRedis.zadd("zset2", "4800", 1)
    print(zinsert)
    
    print("---zcard---")
    zcard = miniRedis.zcard("zset2")
    print(zcard)
    zcard = miniRedis.zcard("zset1")
    print(zcard)
    zcard = miniRedis.zcard("zsetX")
    print(zcard)

    print("---zrank---")
    zrank = miniRedis.zrank("teste10", "mil")
    print(zrank)
    zrank = miniRedis.zrank("zset2", "amil")
    print(zrank)
    zrank = miniRedis.zrank("zset2", "odoismil")
    print(zrank)
    zrank = miniRedis.zrank("zset2", "ctresmil")
    print(zrank)

    print("---zrange---")
    zrange = miniRedis.zrange("zset2", 0, 5)
    print(zrange)
    val = miniRedis.get("zset2")
    print(val)

if __name__== "__main__":
    main()

